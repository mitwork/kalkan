...
Route::post('/documents', [\Mitwork\Kalkan\Http\Actions\StoreDocument::class, 'store'])->name('store-document');

// Шаг 1.1 - Генерация QR-кода
Route::get('/documents/generate-qr-code', [\Mitwork\Kalkan\Http\Actions\GenerateQrCode::class, 'generate'])->name('generate-qr-code');
...
Route::put('/documents/content', [\Mitwork\Kalkan\Http\Actions\ProcessContent::class, 'process'])->name('process-content');

// Шаг 5 - Проверка статуса подписания документа
Route::get('/documents/check/{id}', [\Mitwork\Kalkan\Http\Actions\CheckDocument::class, 'check'])->name('check-document');
