## Шаг 2 - Формирование запроса

На данном шаге необходимо передать параметры запроса и массив документов. В предлагаемой реализации запрос сохраняется в кэш.

### Пример запроса

`POST` /api/requests

```json
{
    "description": "Название запроса",
    "files": [
        {
            "name":"document.docx",
            "data": "base64...",
            "size": 9,
            "mime": "application/vnd.openxmlformats-officedocument.wordprocessingml.document",
            "meta": [
                {
                    "name": "Наименование документа",
                    "value": "Договор для тестирования"
                }
            ]
        }
    ],
    "auth": {
        "type": "None",
        "token": ""
    }
    
}
```

где:

- `description` - наименование запроса для пользователя (необязательно);
- `files` - массив документов из [Шага 1 - загрузка документов](STEP_10_STORE_DOCUMENT.md);
- `auth` - параметры авторизации.

В случае, если документы уже были ранее загружены, пример запроса будет следующим:

```json
{
    "description": "Название запроса",
    "files": [
        {
            "id": 134372667717125,
            "name":"document.docx",
            "data": "base64...",
            "size": 9,
            "mime": "application/vnd.openxmlformats-officedocument.wordprocessingml.document",
            "meta": [
                {
                    "name": "Наименование документа",
                    "value": "Договор для тестирования"
                }
            ]
        }
    ],
    "auth": {
        "type": "None",
        "token": ""
    }

}
```

### Пример ответа

```json
{
    "id": "acba8198-92d9-4297-905f-eb55ea69f9c4",
    "url": "http://localhost/api/requests/generate/acba8198-92d9-4297-905f-eb55ea69f9c4?expires=1697428841&id=134372667717125&signature=388119f23e5cdec7e7c7a58476b3aa7953ec5bb2f6c9b47b411e08222ee5e9eb",
    "links": {
        "qr": {
            "uri": "data:image/png;base64,",
            "raw": ""
        },
        "app": {
            "mobile": "https://mgovsign.page.link/?link=http%3A%2F%2Flocalhost%2Fdocuments%2Fgenerate-link%3Fexpires%3D1697428841%26id%3D134372667717125%26signature%3D388119f23e5cdec7e7c7a58476b3aa7953ec5bb2f6c9b47b411e08222ee5e9eb&isi=1476128386&ibi=kz.egov.mobile&apn=kz.mobile.mgov",
            "business": "https://egovbusiness.page.link/?link=http%3A%2F%2Flocalhost%2Fdocuments%2Fgenerate-link%3Fexpires%3D1697428841%26id%3D134372667717125%26signature%3D388119f23e5cdec7e7c7a58476b3aa7953ec5bb2f6c9b47b411e08222ee5e9eb&isi=1597880144&ibi=kz.mobile.mgov.business&apn=kz.mobile.mgov.business"
        }
    },
    "expire": 1697699037
}
```

где:

- `id` - уникальный идентификатор запроса;
- `url` - сервисная ссылка, используемая при формировании QR-кода и кросс-ссылок;
- `links.qr.uri` - изображение QR-кода для отображения в браузере;
- `links.qr.raw` - изображение QR-кода в исходном формате;
- `links.app.mobile` - кросс-ссылка для подписания в приложении eGov mobile (для индивидуальных предпринимателей и физических лиц);
- `links.app.business` - кросс-ссылка для подписания в приложении eGov business (для юридических лиц);
- `expire` - время жизни ссылки.

### Реализация

Пример реализации [Mitwork\Kalkan\Http\Actions\StoreRequest.php](../src/Http/Actions/StoreRequest.php)

Для использования реализации из текущего пакета необходимо добавить соответсвующий маршрут (`route`) в приложение:

```php
<?php

use Illuminate\Http\Request;
use Illuminate\Support\Facades\Route;

Route::post('/requests', [\Mitwork\Kalkan\Http\Actions\StoreRequest::class, 'store'])->name(config('kalkan.actions.store-request'));
```

Так же, можно реализовать собственную логику данного шага, указав к конфигурации собственный именованный маршрут (`route`):

```php
// ...
'actions' => [
    'store-request' => 'custom-store-request',
]
// ..
```

Либо переопределить используемый класс и метод в маршрутах.
